...
{bitmodule title="$moduleTitle" name="who_is_there"}
	...
	{section name=ix loop=$online_users}
		{if $user && $gBitSystem->isFeatureActive( 'feature_messages' ) && $gBitUser->hasPermission( 'p_messages_send' )}
			<a href="{$smarty.const.MESSAGES_PKG_URL}compose.php?to={$online_users[ix].user}" title="{tr}Send a message to{/tr} {$online_users[ix].user}">{biticon ipackage="icons" iname="internet-mail" iexplain="send message"}</a>
		{/if}
