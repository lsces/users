...
if( !empty( $_REQUEST['home'] ) ) {
	if( $gQueryUser->isValid() && (( $gBitUser->hasPermission( 'p_users_view_user_homepage' ) || $gBitUser->hasPermission( 'p_users_admin' )) || $gQueryUser->mUserId == $gBitUser->mUserId )) {
		...
		$gBitSystem->fatalError( KernelTools::tra( "Page not found." ), null, null, HttpStatusCodes::HTTP_NOT_FOUND );
	}
} else {
	$gBitSystem->verifyPermission( 'p_users_view_user_list' );
	$users = $gQueryUser->getList( $_REQUEST );
	$gBitSmarty->assign( 'users', $users );
	$gBitSmarty->assign( 'usercount', $_REQUEST["cant"] );
	// display an error message
	if( !empty( $_REQUEST['home'] )) {
		$gBitSystem->setHttpStatus( HttpStatusCodes::HTTP_GONE );
		$feedback['error'] = KernelTools::tra( 'The following user could not be found' ).': '.$_REQUEST['home'];
		$gBitSmarty->assign( 'feedback', $feedback );
	}
	$_REQUEST['listInfo']["URL"] = USERS_PKG_URL."index.php";
	$gBitSmarty->assign( 'control', $_REQUEST['listInfo'] );
	$gBitSmarty->assign( 'listInfo', $_REQUEST['listInfo'] );
	$browserTitle = $gBitSystem->getConfig( 'site_title' ).' '.KernelTools::tra( 'Members' );
