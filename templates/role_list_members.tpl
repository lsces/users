			...
			{foreach from=$roleMembers key=userId item=member}
				<li>{displayname hash=$member}
					{if $member.user_id != $smarty.const.ANONYMOUS_USER_ID && $roleInfo.role_id != $smarty.const.ANONYMOUS_TEAM_ID}
						&nbsp;<a href="{$smarty.const.USERS_PKG_URL}admin/assign_role_user.php?action=removerole&amp;role_id={$roleInfo.role_id}&amp;assign_user={$member.user_id}">{biticon ipackage="icons" iname="user-trash" iexplain="remove from role"}</a>
					{/if}
				</li>
			{foreachelse}
