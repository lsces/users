		...
		{jstabs}
			...
			{jstab title="Add User"}
				...
				{form legend="Add a new user" secure=$gBitSystem->isFeatureActive("site_https_login_required")}
					...
					{forminput}
						{$defaultRoleName} <a href="{$smarty.const.USERS_PKG_URL}admin/edit_role.php?role_id={$defaultRoleId}">{biticon ipackage="icons" iname="edit" iexplain="change settings"}</a>
