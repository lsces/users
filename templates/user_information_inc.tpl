...
{if $userData->getPreference('users_information') eq 'public' || $gBitUser->mUserId eq $userData->mUserId}
	...
	{if $userData->getPreference('users_country')}
		<div class="form-group">
			...
			{forminput}
				{biticon ipackage="users" ipath="flags/" iname=$userData->getPreference('flag') iexplain=$userData->getPreference('users_country') iforce="icon" istyle="flag"} {$userData->getPreference('users_country')}
			{/forminput}
		</div>
